...
class DynamicNoisyDataset(Dataset):
    def __init__(self, clean_dir, transform=None, noise_std_range=(5, 50)):
    ...
    def __getitem__(self, idx):
        ...
        # Generate two independent noisy versions
        noise_std_1 = np.random.uniform(*self.noise_std_range) / 255.0
        noise_std_2 = np.random.uniform(*self.noise_std_range) / 255.0
        
        noise_1 = torch.randn_like(clean_tensor) * noise_std_1
        noise_2 = torch.randn_like(clean_tensor) * noise_std_2

        noisy_tensor_1 = clean_tensor + noise_1
        noisy_tensor_2 = clean_tensor + noise_2
...
# ================== Training Code ==================
def train_model(clean_dir, output_model, epochs=10, batch_size=8, lr=1e-3, noise_std_range=(5, 50)):
    ...
    train_dataset = DynamicNoisyDataset(clean_dir, transform=transform, noise_std_range=noise_std_range)
...
if __name__ == "__main__":
    ...
    output_model_path = current_dir / "denoising_autoencoder_n2n_3.pth"

    train_model(clean_images_dir, output_model_path, epochs=300, batch_size=8, lr=1e-2, noise_std_range=(5, 50))
